...
class StaffRecyclerAdapter :
	...
	interface OnClickListener {
		fun onDeleteClicked(staff: Staff)
		fun onUpdateClicked(staff: Staff)
	}
		...
		fun setData(viewBinding: LayoutStaffListBinding, staff: Staff) {
			...
			viewBinding.icMoreVert.setOnClickListener {
				...
				popup.setOnMenuItemClickListener { item ->
					when (item.itemId) {
						R.id.action_delete -> onClickListener.onDeleteClicked(staff)
						R.id.action_update -> onClickListener.onUpdateClicked(staff)
					}
					false
				}
				popup.show()
